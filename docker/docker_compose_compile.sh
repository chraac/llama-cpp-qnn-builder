...
_build_options='-DBUILD_SHARED_LIBS=off -DGGML_QNN_ENABLE_CPU_BACKEND=off -DGGML_OPENMP=off'
...
while (("$#")); do
    ...
    --build-linux-x64)
        ...
        _build_options='-DBUILD_SHARED_LIBS=off -DGGML_QNN_ENABLE_CPU_BACKEND=off -DGGML_OPENMP=off -DLLAMA_SANITIZE_ADDRESS=on'
        shift
        ;;
    --run-tests)
