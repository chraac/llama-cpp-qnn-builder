...
if (-not $TestOnly) {
    $perfLogName = "test-backend-ops-perf-all.release.hexagon.$Revision"
    ...
    Write-Host "Running device performance tests and saving log to $perfLogName"
    & "$scriptPath/run_all_tests_and_save_log.ps1" -e:"perf -b hexagon-npu" -l:"$perfLogName"

    Write-Host "Running device model test and saving log to $modelTestLogName"
...
if (-not $BenchmarksOnly) {
    ...
    Write-Host "Running device tests and saving log to $testLogName"
